...
suite('Share Status', function() {
    ...
    test('Get Status', function(done) {
        async.waterfall([
            function (cb) {
                User.GetStatus('Nancy',function(cb,docs){
    ...
    test('Get Status no Exist', function(done) {
        async.waterfall([
            function (cb) {
                User.GetStatus('Nancy1',function(cb,docs){
    ...
    test('Set Status no Exist', function(done) {
        async.waterfall([
            function (cb) {
                User.SetStatus('Nancy1','2',function(cb,docs){
                    expect(404).to.be.equal(cb);
                    // expect('3').to.be.equal(docs[0].status);
                    done();
                })
    ...
    test('Set Status', function(done) {
        async.waterfall([
            function (callback) {
                User.SetStatus('Nancy','2',function(cb){
            ...
            function (cb) {
                User.GetStatus('Nancy',function (cb,docs) {
                    expect(200).to.be.equal(cb);
                    expect('2').to.be.equal(docs[0].status);
                    done();
                })
            }
        ]);
